...
export async function POST(request) {
  const { R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_BUCKET, R2_PUBLIC_BASE } = process.env;
  // 정상 이름 우선, 없으면 오타 이름(R2_SECRET_ACCES_KEY)도 허용
  const R2_SECRET_ACCESS_KEY = process.env.R2_SECRET_ACCESS_KEY || process.env.R2_SECRET_ACCES_KEY;
